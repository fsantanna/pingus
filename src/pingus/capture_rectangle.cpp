...
CaptureRectangle::set_pingu (Pingu* p)
{
  ...
  if (pingu)
  {
    ...
    if (pingu->wall_action != ActionName::NONE ||
        pingu->fall_action != ActionName::NONE)
    {
      action_str += "[";

      if (pingu->wall_action != ActionName::NONE) {
        char c = ActionName::PERSISTENT_CHAR[pingu->wall_action];
        if (c == 0) {
      ...
      if (pingu->fall_action != ActionName::NONE) {
        char c = ActionName::PERSISTENT_CHAR[pingu->fall_action];
        if (c == 0) {
          assert(!"This is not a persitent action!");
        }
        action_str += c;

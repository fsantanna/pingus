...
GoalManager::GoalManager(Server* s) : server(s), goal(GT_NONE), exit_time(0) { }
...
void GoalManager::update() {
  if (exit_time == 0) {
    ...
    if (pingus->number_of_allowed == pingus->number_of_released && pingus->number_of_alive == 0) {
      goal = GT_NO_PINGUS_IN_WORLD;
    } else if (pingus->number_of_alive == 0 && CEU_World_check_armageddon(NULL,GLOBAL_CEU_WORLD)) {
      goal = GT_ARMAGEDDON;
    } else if (plf.get_time() != -1 && plf.get_time() <= CEU_World_get_time(NULL, GLOBAL_CEU_WORLD)) {
      goal = GT_OUT_OF_TIME;
    }
  }
}

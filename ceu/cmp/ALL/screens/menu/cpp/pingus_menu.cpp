PingusMenu::PingusMenu() :
{
  ...
  start_button = new MenuButton(this, Vector2i(size_.width/2 - 125, size_.height/2 - 20), _("Story"), _("..:: Start the game ::.."));
  editor_button = new MenuButton(this, Vector2i(size_.width/2 + 125, size_.height/2 - 20), _("Editor"), _("..:: Create your own levels ::.."));
  options_button = new MenuButton(this, Vector2i(size_.width/2 + 125, size_.height/2 + 50), _("Options"), _("..:: Configure the game ::.."));
  contrib_button = new MenuButton(this, Vector2i(size_.width/2 - 125, size_.height/2 + 50), _("Levelsets"), _("..:: Play User Built levels ::.."));
  quit_button = new MenuButton(this, Vector2i(size_.width/2, size_.height/2 + 120), _("Exit"), _("..:: Bye, bye ::.."));
...
void PingusMenu::show_credits() {
  ScreenManager::instance()
    ->push_screen(std::make_shared<Credits>(Pathname("credits/pingus.credits", Pathname::DATA_PATH)));
}
void PingusMenu::do_quit() {
  ScreenManager::instance ()->pop_screen ();
}
void PingusMenu::do_start(const std::string &filename) { // Start the story or worldmap mode
...
void PingusMenu::do_contrib(const std::string &levelfile) { // Launch the specified level - don't bother checking for it, it has to exist
  Sound::PingusSound::play_sound ("letsgo");
  ScreenManager::instance()->push_screen (std::make_shared<StartScreen>(PLFResMgr::load_plf_from_filename(Pathname(levelfile, Pathname::SYSTEM_PATH))));
}
void PingusMenu::do_edit() {       // Launch the level editor
...
void PingusMenu::draw_background(DrawingContext& gc) {
  background->draw(gc);
  gc.draw(logo, Vector2i((gc.get_width()/2) - (logo.get_width()/2), gc.get_height()/2 - 280));
  gc.print_left(Fonts::pingus_small, Vector2i(gc.get_width()/2 - 400 + 25, gc.get_height()-140), "Pingus " VERSION " - Copyright (C) 1998-2011 Ingo Ruhnke <grumbel@gmail.com>\n" "See the file AUTHORS for a complete list of contributors.\n" "Pingus comes with ABSOLUTELY NO WARRANTY. This is free software, and you are\n" "welcome to redistribute it under certain conditions; see the file COPYING for details.\n");
  gc.draw_fillrect(Rect(0, Display::get_height () - 26, Display::get_width (), Display::get_height ()), Color(0, 0, 0, 255));
  gc.print_center(Fonts::pingus_small, Vector2i(gc.get_width() / 2, gc.get_height() - Fonts::pingus_small.get_height() - 8), help);
}
void PingusMenu::on_click(MenuButton* button) {
...
void PingusMenu::create_background(const Size& size_) {
  ...
  int w = size_.width;
  int h = size_.height;
  if (w != globals::default_screen_width || h != globals::default_screen_height) {
...
void PingusMenu::resize(const Size& size_) {
  GUIScreen::resize(size_);
  create_background(size);
  start_button->set_pos(size.width/2 - 125, size.height/2 - 20);
  editor_button->set_pos(size.width/2 + 125, size.height/2 - 20);
  contrib_button->set_pos(size.width/2 - 125, size.height/2 + 50);
  options_button->set_pos(size.width/2 + 125, size.height/2 + 50);
  quit_button->set_pos(size.width/2, size.height/2 + 120);
}
